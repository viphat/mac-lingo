...
**Full Xcode is required** (not just Command Line Tools) — `xcodebuild` won't run
otherwise. The Xcode project is **generated by XcodeGen** from `project.yml` (the
source of truth); the `.xcodeproj` is git-ignored, so regenerate it after pulling
...
# Generate the project (after clone, or whenever files are added/removed)
xcodegen generate

# Build (debug) — scheme is "MacLingo"
xcodebuild -scheme MacLingo -configuration Debug -destination 'platform=macOS' build CODE_SIGNING_ALLOWED=NO

# Run tests
xcodebuild -scheme MacLingo -destination 'platform=macOS' test CODE_SIGNING_ALLOWED=NO

# Lint / format (run before every commit) — both must be clean under --strict
swiftlint --strict
swift-format lint --strict --recursive Sources Tests
```

- **Arch split:** Debug builds the **active arch only** (`ONLY_ACTIVE_ARCH=YES`) to
  sidestep an Xcode explicit-modules bug that rejects SwiftPM package
  `.swiftmodule`s in a universal compile; **Release stays universal** for
  distribution (verify the universal/Release build at Phase 8 packaging).
- **Lint division of labour:** swift-format owns whitespace/comma style; SwiftLint
  owns correctness (force-unwrap = error). The `todo` and `trailing_comma` SwiftLint
  rules are disabled (phase-marker TODOs are intentional; swift-format owns commas).
- After a clean of SwiftPM artifacts, `xcodebuild -resolvePackageDependencies` (or
  delete `~/Library/Developer/Xcode/DerivedData/MacLingo-*` and rebuild) if a stale
  package `.swiftmodule` shows up as "built for incompatible target".
...
Progress + the running checklist live in `.agents/todo.md`.
